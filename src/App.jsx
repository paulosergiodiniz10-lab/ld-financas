...
// ==========================================
// ÍCONES TEMPORÁRIOS (Simulando Lucide/Material)
// Num projeto Vercel real, você importará: import { Home, Wallet } from 'lucide-react';
// ==========================================
            ...
            {adminUsers.map(user => (
              <tr key={user.id} className="border-b border-slate-50 hover:bg-slate-50/50 transition-colors">
                ...
                <td className="p-4 flex gap-2">
                  ...
                  >
                    {user.status === 'Ativo' ? <span className="material-symbols-outlined text-base">lock</span> : <span className="material-symbols-outlined text-base">lock_open</span>}
                  </button>
                  <button onClick={() => openConfirm('Em Breve', 'A edição de dados de clientes ficará ativa assim que conectarmos o banco de dados.', closeConfirm, true)} className="p-2 bg-blue-50 text-blue-600 rounded-lg hover:bg-blue-100" title="Editar Usuário"><Edit2 size={16}/></button>
                  <button onClick={() => handleDeleteAdminUser(user.id)} className="p-2 bg-red-50 text-red-600 rounded-lg hover:bg-red-100" title="Excluir Usuário"><Trash2 size={16}/></button>
                </td>
              </tr>
            ))}
            {adminUsers.length === 0 && (
